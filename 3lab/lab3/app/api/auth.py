...
def brut_hash(request: BrutHashRequest, current_user: User = Depends(get_current_user)):
    task_id = start_brutforce(request.hash, request.charset, request.max_length, current_user.id)
